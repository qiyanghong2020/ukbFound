...
class Preprocessor:
    ...
    def __init__(
        ...
        filter_trait_by_counts: Union[int, bool] = False,
        filter_individual_by_counts: Union[int, bool] = False,
        ...
        self.filter_trait_by_counts = filter_trait_by_counts
        self.filter_individual_by_counts = filter_individual_by_counts
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        batch_key (:class:`str`, optional):
            ...
            is used in the highly variable trait selection step.
        """
        ...
        # step 1: filter traits
        if self.filter_trait_by_counts:
            logger.info("Filtering traits by counts ...")
            sc.pp.filter_genes(
                adata,
                min_counts=self.filter_trait_by_counts
                if isinstance(self.filter_trait_by_counts, int)
                else None,
            )

        # step 2: filter individuals
        if (
            isinstance(self.filter_individual_by_counts, int)
            and self.filter_individual_by_counts > 0
        ):
            logger.info("Filtering individuals by counts ...")
            sc.pp.filter_cells(
                adata,
                min_counts=self.filter_individual_by_counts
                if isinstance(self.filter_individual_by_counts, int)
                else None,
            )
        ...
        # step 5: subset hvg
        if self.subset_hvg:
            logger.info("Subsetting highly variable traits ...")
            if batch_key is None:
                logger.warning(
                    "No batch_key is provided, will use all individuals for HVG selection."
                )
            sc.pp.highly_variable_traits(
                adata,
                layer=self.hvg_use_key,
                n_top_traits=self.subset_hvg
            ...
            for row in layer_data:
                if row.max() == 0:
                    logger.warning(
                        ...
                        "this is expected. You can use the `filter_individual_by_counts` "
