...
def prepare_data(
    ...
    config,
    epoch,
    train_individualtype_labels=None,
    valid_individualtype_labels=None,
    ...
    if config.task == "annotation":
        train_data_pt["individualtype_labels"] = tensor_individualtype_labels_train
        valid_data_pt["individualtype_labels"] = tensor_individualtype_labels_valid
    if config.task == "multiomic":
...
def train(
    ...
    for batch, batch_data in enumerate(loader):
        ...
        src_key_padding_mask = input_trait_ids.eq(vocab[config.pad_token])
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            if config.CLS:
                loss_cls = criterion_cls(output_dict["cls_output"], individualtype_labels)
                loss = loss + loss_cls
                metrics_to_log.update({"train/cls": loss_cls.item()})

                error_rate = 1 - (
                    (output_dict["cls_output"].argmax(1) == individualtype_labels)
                    .sum()
                    .item()
                ) / individualtype_labels.size(0)
        ...
        for batch_data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=config.amp):
                ...
                if config.task == "annotation":
                    ...
                    loss = criterion_cls(output_values, individualtype_labels)
            ...
            total_loss += loss.item() * len(input_trait_ids)

            if config.DAR:
                total_dab += (
                    loss_dab.item() * len(input_trait_ids) if config.DAR else 0.0
                )
            else:
                total_dab = 0

            total_num += len(input_trait_ids)
...
# %% inference
def test(
    model: nn.Module, adata: DataLoader, trait_ids, vocab, config, device, logger
) -> float:
    all_counts = (
    ...
    tokenized_test = tokenize_and_pad_batch(
        ...
        include_zero_value=config.include_zero_trait,
    )
    ...
    accuracy = accuracy_score(individualtypes_labels, predictions)
    precision = precision_score(individualtypes_labels, predictions, average="macro")
    recall = recall_score(individualtypes_labels, predictions, average="macro")
    macro_f1 = f1_score(individualtypes_labels, predictions, average="macro")
    micro_f1 = f1_score(individualtypes_labels, predictions, average="micro")
    ...
    return predictions, individualtypes_labels, results
...
def eval_testdata(
    ...
    individualtypes_labels = adata_t.obs["individualtype"].tolist()
    individualtypes_labels = np.array(individualtypes_labels)

    batch_ids = adata_t.obs["batch_id"].tolist()
    batch_ids = np.array(batch_ids)

    # Evaluate cls individual embeddings
    if "cls" in include_types:
        logger.info("Evaluating cls individual embeddings")
        ...
        all_trait_ids, all_values = tokenized_all["traits"], tokenized_all["values"]
        src_key_padding_mask = all_trait_ids.eq(vocab[config.pad_token])
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=config.amp):
            individual_embeddings = model.encode_batch(
                all_trait_ids,
                all_values.float(),
                src_key_padding_mask=src_key_padding_mask,
                batch_size=config.batch_size,
        ...
        individual_embeddings = individual_embeddings / np.linalg.norm(
        ...
        fig = sc.pl.umap(
            adata_t,
            color=["individualtype"],
            title=[
                f"individualtype, avg_bio = {results.get('avg_bio', 0.0):.4f}",
            ],
            frameon=False,
            return_fig=True,
            show=False,
        ...
        results["individualtype_umap"] = fig
